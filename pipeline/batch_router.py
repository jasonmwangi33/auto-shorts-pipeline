...
def run_story_lifecycle(story_id):
    story_key = str(story_id)
    print("==================================================")
    print(f"LIFECYCLE PIPELINE START: Story {story_key}")
    print("==================================================")

    # --- STAGE 1: AI & SPELLING CHECK ---
    print(f"[*] [Stage 1/3] Fetching and spell-checking Story {story_key} via Gemini AI...")
    ...
    print(f"[+] [Stage 1/3 Complete] Story {story_key} spell-checked & titled: '{custom_title}'")

    # --- STAGE 2: RENDER & SUBTITLES ---
    print(f"[*] [Stage 2/3] Rendering video parts with center subtitles for Story {story_key}...")
    ...
    print(f"[+] [Stage 2/3 Complete] All parts rendered for Story {story_key}.")

    # --- STAGE 3: ACCOUNT-ISOLATED PUBLISHING ---
    print(f"[*] [Stage 3/3] Publishing Story {story_key} to assigned YouTube account...")
    try:
    ...
    for idx, local_video_path in enumerate(video_paths):
        ...
        try:
            ...
            print(f"[+] Success: Video pushed to Account #{target_account_index + 1}.")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    story_id_arg = sys.argv[1]
    run_story_lifecycle(story_id_arg)
